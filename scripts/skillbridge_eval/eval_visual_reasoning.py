"""Visual-reasoning held-out evaluation driver (block C4).

Wraps :mod:`cold_start.generate_cold_start_actor_visual_reasoning`, which
already iterates the four visual-reasoning benchmarks
(MathVerse, MMMU-Pro, VisualToolBench, TIR-Bench) through a vision ->
schema -> actor pipeline using a single ``--model`` slug.
...
_BENCHMARKS = ("visual_toolbench", "tir_bench")
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        help=f"Subset of visual-reasoning benchmarks "
             f"(default: all of {_BENCHMARKS}).",
    )
    p.add_argument("--num-test-cases", type=int, default=200)
    p.add_argument(
        "--sample-ids-dir", type=Path, default=None,
        help="Optional manifest dir produced by "
             "build_visual_reasoning_diverse_1000.py.",
    )
    ...
    p.add_argument(
        ...
        help="Enable LLM-as-judge for VisualToolBench / TIR-Bench.",
    )
...
def _aggregate(out_dir: Path) -> Dict[str, Any]:
    ...
    for entry in data.get("per_benchmark", []) or []:
        ...
        per_bench[name] = {
            ...
            "schema_ok": int(entry.get("schema_ok", 0)),
            "answer_ok": int(entry.get("answer_ok", 0)),
            "correct_ok": int(correct),
            "correct_total_with_gold": int(with_gold),
            "accuracy": (correct / with_gold) if with_gold else None,
            "judge_enabled": bool(entry.get("judge_enabled", False)),
        }
...
def main() -> int:
    ...
    out_dir = args.run_dir / "eval" / f"visual_reasoning_{ts}"
    ...
    logger.info("eval_visual_reasoning: $ %s", " ".join(cmd))
    t0 = time.time()
    proc = subprocess.run(cmd, env=env)
    wall = time.time() - t0
    logger.info(
        "eval_visual_reasoning: cold-start exit=%d wall=%.1fs",
    ...
    out_path = args.output or (
        args.run_dir / "eval" / f"visual_reasoning_result_{ts}.json"
    )
    out_path.parent.mkdir(parents=True, exist_ok=True)
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(result, f, indent=2, default=str)
    logger.info("eval_visual_reasoning: wrote %s", out_path)

    print("\n=== visual-reasoning eval summary ===")
