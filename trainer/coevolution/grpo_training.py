...
# ── Ablation hooks (env-var driven, zero-risk for production) ─────────
# Two env vars allow ablation runs to disable / merge LoRA adapters
# without modifying the orchestrator or config dataclass:
#
#   COSPLAY_DISABLE_ADAPTERS="curator,contract"
#     Comma-separated list. Those adapters are skipped in
#     build_jobs() so they keep their cold-start (SFT) weights
#     for the entire run.
#
#   COSPLAY_MERGE_ADAPTERS="decision" | "skillbank" | "all"
#     Reroutes every record / sample destined for the adapters in
#     the named group to a single shared adapter ("decision_merged",
#     "skillbank_merged", or "all_merged" respectively). Used to test
#     "is the 2-/3-/5-way split essential?". Off by default.
...
def _disabled_adapters() -> set:
    raw = os.environ.get("COSPLAY_DISABLE_ADAPTERS", "")
    return {x.strip() for x in raw.split(",") if x.strip()}


def _merge_target() -> Optional[str]:
    """Return the active merge-group name from the env var, or None."""
    group = os.environ.get("COSPLAY_MERGE_ADAPTERS", "").strip().lower()
    return group if group in _ABLATION_GROUPS else None


def _reroute_for_merge(records: Dict[str, list]) -> Dict[str, list]:
    """If COSPLAY_MERGE_ADAPTERS is set, concatenate records of every
    member adapter into a single ``<group>_merged`` key."""
    group = _merge_target()
    if not group:
        return records
    members = _ABLATION_GROUPS[group]
    pooled: list = []
    for name in members:
        pooled.extend(records.get(name, []))
    out = dict(records)
    for name in members:
        out.pop(name, None)
    out[f"{group}_merged"] = pooled
    return out
...
class DecisionGRPOTrainer:
    ...
    def build_jobs(
        ...
        # ── Ablation: merge / disable adapters ─────────────────────────
        records = _reroute_for_merge(records)
        merge_group = _merge_target()
        if merge_group in ("decision", "all"):
            target = f"{merge_group}_merged"
            adapter_configs = {target: {
                "lr": self.lr,
                "kl_coeff": self.kl_coeff,
                "base_epochs": 3,
            }}
        disabled = _disabled_adapters()
        if disabled:
            adapter_configs = {
                k: v for k, v in adapter_configs.items() if k not in disabled
            }
            if disabled & {"skill_selection", "action_taking"}:
                logger.warning(
                    "[ablation] Decision adapters disabled this run: %s",
                    sorted(disabled & {"skill_selection", "action_taking"}),
                )
...
class SkillBankGRPOTrainer:
    ...
    def build_jobs(
        ...
        # ── Ablation: merge / disable adapters ─────────────────────────
        grpo_data = _reroute_for_merge(grpo_data)
        merge_group = _merge_target()
        if merge_group in ("skillbank", "all"):
            target = f"{merge_group}_merged"
            adapter_configs = {target: {
                "lr": self.lr,
                "kl_coeff": self.kl_coeff,
        ...
        if disabled:
            ...
            if disabled & {"segment", "contract", "curator"}:
                logger.warning(
                    "[ablation] Skill-bank adapters disabled this run: %s",
                    sorted(disabled & {"segment", "contract", "curator"}),
                )
...
async def run_grpo_training(
    ...
    *,
    ...
    skillbank_disabled = bool(getattr(config, "disable_skillbank", False))
    skillbank_frozen = bool(getattr(config, "freeze_skillbank", False))

    # Ablation: when the skill bank is disabled, train only action_taking.
    # Skill_selection has no signal (no skill retrieval happened in
    # rollouts) and the three skill-bank adapters have no data.
    if skillbank_disabled:
        decision_records = {
            "action_taking": decision_records.get("action_taking", []),
        }
    # Ablation: when the skill bank is FROZEN, train the full decision
    # agent (action_taking AND skill_selection — rollouts DID use the
    # bank so skill_selection has signal), but skip the skill-bank
    # trainer (segment/contract/curator stay frozen).
    ...
    if skillbank_disabled:
        logger.info(
            "Phase C: skill-bank GRPO trainer SKIPPED "
            "(--no-skillbank ablation)"
        )
    elif skillbank_frozen:
        logger.info(
            "Phase C: skill-bank GRPO trainer SKIPPED "
            "(--freeze-skillbank ablation; segment/contract/curator frozen)"
        )
    else:
