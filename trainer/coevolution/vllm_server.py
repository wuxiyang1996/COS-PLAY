...
class VLLMServerManager:
    ...
    def _launch_wave(
        ...
        # Stagger launches to dodge the documented race in
        # vllm/config/parallel.py:562 — concurrent uniproc engines all
        # probe `get_open_port()` simultaneously, collide on the same
        # distributed-init port, and 3/4 die with EADDRINUSE.
        # Override with COSPLAY_VLLM_LAUNCH_STAGGER_S=0 for legacy timing.
        stagger_s = float(os.environ.get(
            "COSPLAY_VLLM_LAUNCH_STAGGER_S", "3.0"))

        for idx_in_wave, (i, gpu_id) in enumerate(entries):
            ...
            # Push each instance's distributed-init probe into a private
            # high-port range so even concurrent probes cannot collide.
            env["VLLM_PORT"] = str(20000 + i * 1000)
            ...
            logger.info(
                ...
                "(VLLM_PORT=%s for distributed init)",
            ...
            # Stagger between Popens within the wave (not after the last).
            if stagger_s > 0 and idx_in_wave < len(entries) - 1:
